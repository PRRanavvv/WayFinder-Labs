...
export function scoreTravelCandidate({
  ...
  const decisionScore = weightedAverage(scoreBreakdown, scoringConfig.weights);
  const decisionReasons = explainDecision(candidate, scoreBreakdown);
  const confidence = scoreConfidence(scoreBreakdown, decisionScore);
  ...
  return {
    ...
    scoreBreakdown,
    scoreContributions: scoreContributions(scoreBreakdown, scoringConfig.weights),
    decisionReasons,
    explanation: `${candidate.name} is recommended because ${decisionReasons.strengths.slice(0, 2).join(" and ")}.`
  };
}

export function summarizeRetrievedContext(retrievedContext) {
...
function scoreContributions(scoreBreakdown, weights) {
  return Object.fromEntries(
    Object.entries(scoreBreakdown).map(([dimension, score]) => [
      dimension,
      Number((score * (weights[dimension] ?? 0)).toFixed(2))
    ])
  );
}

function scoreConfidence(scoreBreakdown, decisionScore) {
  const weakFactors = Object.values(scoreBreakdown).filter((score) => score < 55).length;
  if (decisionScore >= 78 && weakFactors <= 1) return "high";
  if (decisionScore >= 64 && weakFactors <= 2) return "medium";
  return "low";
}
