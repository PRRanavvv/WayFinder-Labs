...
from app.engines.group_preference_fusion import build_trip_dna, score_place_against_trip_dna
...
class GroupIntelligenceTests(unittest.TestCase):
    def test_group_fusion_stability_and_explanations(self):
        ...
        self.assertTrue(
            any(target["preference"] == "nightlife" for target in fusion["fairness"]["representationTargets"])
        )
        ...
        self.assertFalse(
            any(activity.get("id") == "kerala_002" for activity in stable["days"][affected_day - 1]["activities"])
        )
