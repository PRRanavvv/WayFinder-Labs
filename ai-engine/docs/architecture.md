...
- Request contracts: `app/contracts/`
- Seed place provider and route profiles: `app/providers/static_catalog.py`
- Replaceable provider protocol: `app/providers/base.py`
- Group Preference Fusion: `app/engines/group_preference_fusion.py`
- Retrieval primitives: `app/engines/retrieval_engine.py`
- Recommendation ranking: `app/engines/recommendation_engine.py`
- Deterministic itinerary planning: `app/engines/itinerary_planner.py`
- Stability/minimal-change replanning: `app/engines/stability_engine.py`
- Explanation cards: `app/engines/explanation_engine.py`
- Confidence scoring: `app/engines/confidence.py`
- Evaluation baseline: `evals/`
- Active tests: `tests/unit/`, `tests/contract/`, and `tests/evaluation/`
...
- Geo intelligence inputs: route zone, locality cluster, and backtracking group. Verified coordinates remain in the main application.
...
- Visual grounding references supplied by trusted providers when available.
...
The current seed is deliberately small and India-focused. It is a deterministic baseline, not a claim of global production coverage.
...
Destination request
  ...
  -> trusted visual grounding outside the public AI response
  -> cluster generation
...
- Google Places in the main application for verified place identity and coordinates.
- OpenStreetMap/Overpass may support offline research, POI types, and spatial density.
...
- Day-by-day plan.
- Activities with title, locality, time, slot, duration, budget, source, tags, and verification hints. Coordinates are excluded.
