...
def stabilize_itinerary(
    *,
    ...
    if not previous or not previous.get("days") or not next_plan or not next_plan.get("days"):
        ...
        return {
            ...
            **calculate_confidence(
                base=(next_plan or {}).get("confidence", 0.72),
                output_completeness=1 if (next_plan or {}).get("days") else 0.4,
            ),
        }
    ...
    confidence = calculate_confidence(
        ...
        output_completeness=report["currentActivityCount"] / report["previousActivityCount"]
        if report["previousActivityCount"]
        else 1,
    )
...
def infer_affected_days(previous: dict, changes: dict, force_fresh_days: list[int]) -> set[int]:
    ...
    if (
        "*" in weather
        or normalize_text(weather.get("condition") if isinstance(weather, dict) else weather).find("rain") >= 0
    ):
        for day in days:
            affected.add(day["day"])
...
def find_matching_previous_day(previous_days: list[dict], next_day: dict) -> dict | None:
    return next((day for day in previous_days if day["day"] == next_day["day"]), None) or next(
        (
            ...
            if day.get("routeLocation") and day.get("routeLocation") == next_day.get("routeLocation")
        ),
        None,
    )
