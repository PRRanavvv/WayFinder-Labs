...
from .data import DESTINATION_REGION_PROFILES, enriched_travel_places
...
def plan_deterministic_itinerary(
    *,
    ...
    scheduled_days = sum(1 for day in planned_days if any(activity["kind"] == "place" for activity in day["activities"]))
...
def replan_deterministic_itinerary(
    *,
    ...
    if normalize_text((base.get("weather") or {}).get("condition") if isinstance(base.get("weather"), dict) else base.get("weather")) == "rain":
    ...
    if changes.get("flightDelayHours") or changes.get("flightDelayMinutes"):
        ...
        base_start = DEFAULT_ITINERARY_PLANNER_CONFIG["parentDayStartTime"] if base["group"].get("parents") else DEFAULT_ITINERARY_PLANNER_CONFIG["dayStartTime"]
...
def validate_deterministic_itinerary(itinerary: dict) -> dict:
    ...
    return {"valid": all(report["valid"] for report in reports), "dayReports": reports, "routeValid": has_no_backtracking(itinerary.get("days", []))}


def derive_trip_constraints(destination, days, budget, group, preferences, weather, month, schedule_adjustments, config):
    traveler_type = "parents" if group.get("parents") else "backpackers" if group.get("backpackers") or normalize_text(preferences.get("travelStyle")) == "backpacker" else "balanced"
    ...
    max_fatigue = {"parents": config["parentMaxFatigue"], "backpackers": config["backpackerMaxFatigue"]}.get(traveler_type, config["defaultMaxFatigue"])
    max_walking = {"parents": config["parentMaxWalkingHours"], "backpackers": config["backpackerMaxWalkingHours"]}.get(traveler_type, config["defaultMaxWalkingHours"])
    max_activities = {"parents": config["parentMaxActivitiesPerDay"], "backpackers": config["backpackerMaxActivitiesPerDay"]}.get(traveler_type, config["maxActivitiesPerDay"])
...
def place_matches_destination(place: dict, destination: str) -> bool:
    ...
    if profile:
        ...
        return route in {normalize_text(location) for location in profile["locations"]} or normalize_text(place.get("state")) == normalized
...
def score_itinerary_place(place: dict, constraints: dict) -> float:
    ...
    requested_group = "family" if constraints["group"].get("parents") else normalize_text(constraints["preferences"].get("groupType"))
    estimated_cost = estimate_group_cost(place, constraints)
    budget_pressure = 0 if constraints["dailyBudgetCap"] == float("inf") else estimated_cost / constraints["dailyBudgetCap"]
    score = 48
...
def preference_bonus(place: dict, preferences: dict) -> int:
    terms = [*preferences.get("interests", []), *preferences.get("moods", []), preferences.get("vibe"), preferences.get("travelStyle")]
    normalized_terms = [normalize_text(term) for term in terms if term]
    if not normalized_terms:
        return 0
    place_terms = {normalize_text(value) for value in [place["category"], *place.get("mood", []), *place.get("tags", []), *place.get("retrieval_terms", [])]}
...
def select_route_locations(candidates: list[dict], destination: str, days: int) -> list[str]:
    ...
    selected = [
        location
        for location, _candidate in sorted(best_by_location.items(), key=lambda item: item[1]["itineraryScore"], reverse=True)[: max(1, min(days, len(best_by_location)))]
    ]
    ...
    if profile and profile["gatewayLocation"] in best_by_location:
        selected = [profile["gatewayLocation"], *[location for location in selected if location != profile["gatewayLocation"]]]
...
def optimize_route_order(locations: list[str], destination: str) -> list[str]:
    ...
    if profile:
        ...
        return sorted(unique, key=lambda location: order.index(normalize_text(location)) if normalize_text(location) in order else 999)
    return unique
...
def plan_single_day(day_number, destination, route_location, previous_location, candidates, used_place_ids, constraints, skipped_candidates, decision_trace):
    day_start = resolve_day_start(day_number, constraints)
    day_end = resolve_day_end(day_number, constraints)
    state = {"current": to_minutes(day_start), "totalCost": 0, "totalFatigue": 0.0, "totalWalkingHours": 0.0, "activities": []}
    ...
    for candidate in day_candidates:
        if len([activity for activity in state["activities"] if activity["kind"] == "place"]) >= constraints["maxActivitiesPerDay"]:
            break
        attempt = try_schedule_place(candidate, route_location, state, constraints, day_end)
        if not attempt["feasible"]:
            skipped_candidates.append({"day": day_number, "id": candidate["id"], "name": candidate["name"], "reason": attempt["reason"]})
            continue
...
def add_transfer_if_needed(state, day_number, destination, previous_location, route_location, constraints):
    ...
    transfer_start = to_minutes(constraints["earlyTransferStartTime"]) if travel_minutes >= constraints["longTransferThresholdMinutes"] else state["current"]
...
def try_schedule_place(place, route_location, state, constraints, day_end):
    ...
    start = preferred if preferred >= earliest and preferred + duration <= closing and preferred + duration <= to_minutes(day_end) else earliest
    end = start + duration
    ...
    if state["totalFatigue"] + fatigue > constraints["maxDailyFatigue"]:
        return {"feasible": False, "reason": f"{place['name']} would exceed max fatigue {constraints['maxDailyFatigue']}."}
    if state["totalWalkingHours"] + walking_hours > constraints["maxDailyWalkingHours"]:
        return {"feasible": False, "reason": f"{place['name']} would exceed walking limit {constraints['maxDailyWalkingHours']}h."}
    if state["totalCost"] + cost > constraints["dailyBudgetCap"]:
        return {"feasible": False, "reason": f"{place['name']} would exceed daily budget cap {constraints['dailyBudgetCap']}."}
    return {
        "feasible": True,
...
def build_route_summary(days, destination):
    ...
    transfers = [
        {"from": locations[index - 1], "to": locations[index], "estimatedMinutes": estimate_travel_minutes(locations[index - 1], locations[index], destination)}
