...
def rank_destination_recommendations(
    *,
    ...
    source_places = places or enriched_travel_places()
    ...
    for place, retrieval_record in resolved:
        ...
        breakdown = {
            ...
            "preferenceFit": preference_fit(place, userPreferences or user_preferences or {}, travelStyle or travel_style or {}),
...
def preference_fit(place: dict, preferences: dict, travel_style: dict) -> float:
    interests = [*(preferences.get("interests") or []), *(preferences.get("moods") or []), travel_style.get("pace"), travel_style.get("primaryIntent")]
    interests = [interest for interest in interests if interest]
    if not interests:
        return 70
    place_terms = set(tokenize(" ".join([
        place.get("category", ""),
        *place.get("mood", []),
        *place.get("tags", []),
...
def calculate_group_satisfaction(place: dict, members: list[dict]) -> dict:
    ...
    for member in members:
        score = preference_fit(place, {"interests": member.get("interests", []), "moods": member.get("moods", [])}, member.get("travelStyle", {}) if isinstance(member.get("travelStyle"), dict) else {})
