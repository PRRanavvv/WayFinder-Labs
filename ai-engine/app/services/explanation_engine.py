...
def explain_stable_replan(stable_itinerary: dict) -> dict:
    ...
    return {
        "kind": "stable-replan",
        "headline": f"{preserved}/{previous} accepted places were preserved" if previous else "Fresh itinerary generated",
        "selectedBecause": [
            item
            for item in [
                report.get("rule"),
                f"Preserved days: {', '.join(map(str, report.get('preservedDays', [])))}" if report.get("preservedDays") else None,
                f"Regenerated days: {', '.join(map(str, report.get('regeneratedDays', [])))}" if report.get("regeneratedDays") else None,
            ]
            if item
        ],
        "whoThisServes": [
        ...
        "watchouts": [f"Affected days: {', '.join(map(str, report.get('affectedDays', [])))}"] if report.get("affectedDays") else [],
        "confidenceNarrative": explain_confidence(stable_itinerary),
    }


def explain_recommendation_selection(recommendation: dict, *, tripDNA: dict | None = None, trip_dna: dict | None = None) -> dict:
    dna = tripDNA or trip_dna or {}
    group_scores = recommendation.get("groupSatisfaction", {}).get("memberScores", [])
    top_members = [
        member.get("memberId") or member.get("name")
        for member in group_scores
        if float(member.get("score", 0)) >= 70
    ][:3]
    ...
    top_dimensions = [
        dimension
        for dimension, _score in sorted(breakdown.items(), key=lambda item: item[1], reverse=True)[:3]
    ]
    return {
        ...
        "whoThisServes": [f"Good fit for {member}" for member in top_members] if top_members else ["Good shared fit for the group"],
...
def build_who_this_serves(consensus: list[dict]) -> list[str]:
    ...
    return [f"Reflects {member}'s stated preferences" for member in unique] if unique else ["Reflects the strongest shared group signals"]


def explain_confidence(output: dict) -> str:
    level = output.get("confidenceLevel", "medium")
    reasons = output.get("confidenceReasons", [])
    return f"{level} confidence because {'; '.join(reasons[:2])}." if reasons else f"{level} confidence from deterministic scoring signals."
