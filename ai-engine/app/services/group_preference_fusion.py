...
def fuse_group_preferences(
    *,
    ...
    conflict_level = (
        ...
        if conflicts or len(fairness["minorityPreferences"]) >= 3
        else "low"
    )
    ...
    return {
        ...
        "fairness": fairness,
        "conflicts": conflicts,
        "recommendationHints": build_recommendation_hints(soft_preferences, group_vibe, pace_profile, budget_profile, context, members),
        **confidence,
    }
...
def build_fairness_profile(members: list[dict], positive: dict, member_count: int) -> dict:
    minority = [
        public_term(term)
        for term in rank_terms(positive)
        if term["supporterCount"] == 1 and member_count >= 3
    ]
    targets = [
        {
...
def detect_group_conflicts(
    *,
    ...
    conflicts: list[dict] = []
    add_opposition(conflicts, terms, {"quiet", "peaceful", "slow", "calm"}, {"nightlife", "party", "energetic", "social"}, {
    ...
    add_opposition(conflicts, terms, {"adventure", "trek", "hiking", "wild"}, {"low fatigue", "accessible", "parents", "comfort"}, {
    ...
    add_opposition(conflicts, terms, {"budget", "cheap", "low cost"}, {"luxury", "premium", "comfort"}, {
        "type": "budget-comfort",
...
def build_hard_constraints(avoid_terms: list[dict], budget_profile: dict, members: list[dict], context: dict) -> list[dict]:
    ...
    if avoid_terms:
        constraints.append({"type": "avoid", "values": [term["term"] for term in avoid_terms], "source": "traveler dislikes"})
    member_constraints = [
    ...
    if member_constraints:
        constraints.append({"type": "traveler-constraint", "values": sorted(set(member_constraints)), "source": "member constraints"})
    return constraints
...
def build_recommendation_hints(
    ...
    group_type = normalize_text(context.get("groupType")) or ("family" if family else "friends" if len(members) >= 3 else "solo")
    return {
        "groupType": group_type,
        "groupVibe": group_vibe,
        "userPreferences": {
            "interests": soft_preferences,
            "moods": [term for term in soft_preferences if term in {"slow", "quiet", "social", "romantic", "adventurous"}],
        },
        "travelStyle": {"pace": pace_profile["dominantPace"], "primaryIntent": soft_preferences[0] if soft_preferences else None},
        "budgetConstraints": {"maxBudget": budget_profile.get("sharedCap"), "preferredBudget": budget_profile.get("median")},
        "rankingNotes": [
